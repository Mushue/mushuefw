...
final class Binding
{
    ...
    {
        return (string) array_value($this->binding[self::FACTORY]);
    }
     ...
     *            Decorator receives the object instance as first argument and must return an instance of the same type.
     * @return Binding
     */
    public function decorate(callable $decorator)
    {
        $this->binding[self::DECORATORS][] = [
            $decorator
        ];
     ...
     *
     * @param string $typeName
     *            Fully-qualified name of the marker class.
     * @param array $data
     *            Property values to be passed to the marker instance as it is being created.
     * @return Binding
     */
    public function markedOld($typeName, array $data = [])
    {
        if (empty($this->binding[self::MARKERS])) {
    ...
    {
        $this->binding[self::MARKERS][] = $marker;
    ...
    {
        return (array) $this->binding[self::MARKERS];
    }
}